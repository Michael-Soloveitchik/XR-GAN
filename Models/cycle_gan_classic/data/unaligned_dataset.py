...
class UnalignedDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.A_paths = sorted(make_dataset(os.path.join(self.dir_A, opt.input_classes), opt.max_dataset_size))   # load images from '/path/to/data/trainA'
        self.test_A_paths = sorted(make_dataset(os.path.join(self.dir_A_test, opt.input_classes), opt.max_dataset_size))   # load images from '/path/to/data/trainA'
    ...
    def __getitem__(self, index):
        ...
        for clss in self.opt.output_classes.split('_'):
            ...
            B[clss] = self.transform_B(B_img)
        A_img = Image.open(A_path).convert('RGB')
        test_A_img = Image.open(test_A_paths).convert('RGB')
        # apply image transformation
        A = self.transform_A(A_img)
        test_A = self.transform_test_A(test_A_img)
        return {'A': A, 'test_A':test_A, 'A_paths': A_path, 'test_A_paths':test_A_paths} | {'B_paths'+k:v for k,v in B_path.items()} | {'B_'+k:v for k,v in B.items()} | {'B':torch.cat([B[clss] for  clss in self.opt.output_classes.split('_')])}
    def collate(self, batch):
        elem = batch[0]
        a={k:(torch.stack([d[k]for d in batch]) if type(batch[0][k]) != str else np.stack([np.array(d[k]) for d in batch])) for k in elem if 'test' not in k}
