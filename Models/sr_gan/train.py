...
def validate(valid_dataloader, epoch, stage) -> float:
    ...
    with torch.no_grad():
        ...
        if epoch%2==0:
            sr = (sr*256.)[0]
            output = np.transpose(np.concatenate([sr.detach().cpu().numpy(),hr[0].detach().cpu().numpy()*256.],axis=2), [1,2,0])
            os.path.exists(images_dir1) or os.makedirs(images_dir1)
            cv2.imwrite(os.path.join(images_dir1,"outpur_{epoch}.jpg".format(epoch=epoch)),output)
